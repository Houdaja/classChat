 ...
 */
 ...
 echo '<ul>';
 foreach($chat_1->getInfos() as $donnee){
 	echo '<li>' . $donnee . '</li>';
 }
 echo '</ul>';
...
// met pas le $ devant, par contre on le met quand on le déclare
 ...
 $chat_2 = new Chat('Sushi', 1, 'Noire', 'femelle', 'Gouttière');
echo '<strong>Chat 2</strong>';
echo '<ul>';
foreach ($chat_2->getInfos() as $donnee) {
    echo '<li>' . $donnee . '</li>';
}
echo '</ul>';
...
echo '<ul>';
foreach ($chat_3->getInfos() as $donnee) {
    echo '<li>' . $donnee . '</li>';
}
echo '</ul>';
